...
class EventDetailSheet extends StatelessWidget {
  ...
  Color _visibilityColor(ColorScheme cs, EventVisibility visibility) {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ScheduleBloc, ScheduleState>(
      builder: (context, state) {
        final currentEvent = (state is ScheduleLoaded)
            ? state.allEvents.firstWhere(
                (e) => e.id == event.id,
                orElse: () => event,
              )
            : event;
        ...
        final visColor = _visibilityColor(cs, currentEvent.visibility);
            ...
            children: [
                  ...
                  children: [
                    Container(
                      ...
                      child: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Icon(_visibilityIcon(currentEvent.visibility), size: 14, color: visColor),
                          const SizedBox(width: 4),
                          Text(
                            _visibilityLabel(currentEvent.visibility),
                            style: TextStyle(
                              fontSize: 12,
                    ...
                    if (currentEvent.isOfflineCreated) ...[
              ...
              Padding(
                ...
                child: Align(
                  ...
                  child: Text(
                    currentEvent.visibility == EventVisibility.secret
                        ? '🔒 비밀 일정'
                        : currentEvent.title,
                    style: TextStyle(
                      fontSize: 22,
              ...
              _InfoRow(
                ...
                label: '시작',
                value: timeFormat.format(currentEvent.startTime),
                cs: cs,
              ),
              _InfoRow(
                icon: Icons.schedule_outlined,
                label: '종료',
                value: timeFormat.format(currentEvent.endTime),
                cs: cs,
              ),
                    ...
                    Expanded(
                      ...
                      child: FilledButton.icon(
                        onPressed: () {
                          ...
                          context.push('/event/edit', extra: currentEvent);
                        },
  ...
  void _confirmDelete(BuildContext context, CalendarEvent targetEvent) {
    ...
    showDialog<void>(
      ...
      builder: (dialogCtx) => AlertDialog(
        ...
        content: Text(
          '"${targetEvent.title.isEmpty ? '비밀 일정' : targetEvent.title}"을 삭제할까요?\n이 작업은 되돌릴 수 없습니다.',
        ),
        actions: [
          TextButton(
          ...
          FilledButton(
            onPressed: () {
              bloc.add(ScheduleEventDeleted(targetEvent.id));
